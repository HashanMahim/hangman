...
let maxWrongGuesses = 6; // Total number of hangman images
...
function guessLetter(letter, game) {
  if (game.word.includes(letter)) {
    // If the guessed letter is correct, reveal it in the word
    revealLetter(letter, game);
  } else {
    // If the guessed letter is wrong, update the image
    wrongGuesses++;
    updateHangmanImage();
  }
...
function newGame() {
  wrongGuesses = 0;
  updateHangmanImage();
  // Reset other game elements (word, guesses, etc.)
  initializeGame();
}
